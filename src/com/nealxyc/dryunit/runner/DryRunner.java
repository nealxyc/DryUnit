...
//		}
	...
	protected  ParameterResolver createParameterResolver(Class<?> cls) throws ParamResolveException{
	...
	protected List<FrameworkMethod> computeDryTestMethods(List<FrameworkMethod> fwMethods) throws ParamResolveException{
	...
	protected List<FrameworkMethod> computeDryTestMethods(FrameworkMethod fwMethod) throws ParamResolveException{
		if(this.resolver == null){
			this.resolver = createParameterResolver(this.getTestClass().getJavaClass()) ;
		}
		List<FrameworkMethod> list = Lists.newArrayList();
		if(isDryTestMethod(fwMethod)){
			ParamRef[] refs = ParamRefImpl.Helper.getFromMethod(fwMethod.getMethod());
			Collection<Object[]> params = resolver.resolve(fwMethod.getAnnotation(ParamTest.class), refs);
			Iterator<Object[]> itr = params.iterator();
			for(int i = 0 ;i < params.size(); i ++){
				list.add(new DryFrameworkMethod(fwMethod.getMethod(), i, itr.next()));
			}
