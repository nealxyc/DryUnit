...
public class ParamDefinition {
	...
	public static class Helper{
		...
		public static ParamDefinition[] getFromField(Object obj, Field field) throws ParamResolveException{
			...
			if(paramValuesAnno.size() > 0){
				ParamDefinitionValue value = new ParamDefinitionValue().setParamResolveType(ParamResolveType.FIELD);
				
				Object fieldValue = null;
				try {
					fieldValue = field.get(obj);
				} catch (IllegalArgumentException e) {
					throw new ParamResolveException(String.format("Cannot retreive value from field: %s.%s", field.getDeclaringClass().getSimpleName(), field.getName()), e);
				} catch (IllegalAccessException e) {
					throw new ParamResolveException(String.format("Cannot retreive value from field: %s.%s", field.getDeclaringClass().getSimpleName(), field.getName()), e);
				}
				List<Object> resolvedValue = new ArrayList<Object>();
				Class<?> elemType = Object.class ; //default element type is object
				...
				if(fieldValue != null){
					if(fieldValue.getClass().isArray()){
						...
						throw new ParamResolveException(String.format("Target field is not Collection, Array, or Iterable type: %s.%s", field.getDeclaringClass().getSimpleName(), field.getName()));
					}
				}else{
					//null
				}
...
//		public static ParamDefinition[] getFromMethod(Object obj, Method method){
//		    
//		}
	}
}
